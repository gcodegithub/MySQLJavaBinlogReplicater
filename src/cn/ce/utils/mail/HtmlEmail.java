 ...
 * 
 * <p>
 * This class also inherits from MultiPartEmail, so it is easy to add
 * attachments to the email.
 * 
 * <p>
 * To send an email in HTML, one should create a HtmlEmail, then use the
 * setFrom, addTo, etc. methods. The HTML content can be set with the setHtmlMsg
 * method. The alternative text content can be set with setTextMsg.
 * 
 * <p>
 ...
 * @version $Id: HtmlEmail.java,v 1.1 2011/03/25 03:13:01 wangjing Exp $
 */
public class HtmlEmail extends MultiPartEmail {
	...
	/**
	 * Text part of the message. This will be used as alternative text if the
	 ...
	 * 
	 * <p>
	 * This method overrides the MultiPartEmail setMsg() method in order to send
	 * an HTML message instead of a full text message in the mail body. The
	 * message is formatted in HTML for the HTML part of the message, it is let
	 * as is in the alternate text part.
	 * 
	 * @param msg
	 *            A String.
	 * @return An Email.
	...
	public Email setMsg(String msg) throws EmailException {
		...
		setHtmlMsg(new StringBuffer().append("<html><body><pre>").append(msg)
				.append("</pre></body></html>").toString());
	 ...
	 * It allows, for instance, to add inline images to the email. Inline files
	 * may be referenced with a <code>cid:xxxxxx</code> URL, where xxxxxx is the
	 * Content-ID returned by the embed function.
	 * 
	 * <p>
	 * Example of use:<br>
	 * <code><pre>
	 ...
	 *            The URL of the file.
	 * @param name
	 *            The name that will be set in the filename header field.
	 * @return A String with the Content-ID of the file.
	 * @throws EmailException
	 *             when URL supplied is invalid also see
