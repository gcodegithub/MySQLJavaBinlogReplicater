...
public final class RowsLogBuffer {
	...
	final Serializable fetchValue(int type, final int meta, boolean isBinary) {
		...
		case BinlogEvent.MYSQL_TYPE_DATETIME: {
			...
			if (i64 == 0) {
				...
				value = String.format("%04d-%02d-%02d %02d:%02d:%02d",
						d / 10000, (d % 10000) / 100, d % 100, t / 10000,
						(t % 10000) / 100, t % 100);
			}
			javaType = Types.TIMESTAMP;
			length = 8;
		...
		case BinlogEvent.MYSQL_TYPE_DATETIME2: {
			 ...
			 * (year 0-9999, month 0-12) 5 bits day (0-31) 5 bits hour (0-23) 6
			 * bits minute (0-59) 6 bits second (0-59) 24 bits microseconds
			 * (0-999999) Total: 64 bits = 8 bytes SYYYYYYY.YYYYYYYY
		...
		case BinlogEvent.MYSQL_TYPE_TIME2: {
			/*
			 * TIME low-level memory and disk representation routines In-memory
			 * format: 1 bit sign (Used for sign, when on disk) 1 bit unused
			 * (Reserved for wider hour range, e.g. for intervals) 10 bit hour
			 * (0-836) 6 bit minute (0-59) 6 bit second (0-59) 24 bits
			 * microseconds (0-999999) Total: 48 bits = 6 bytes
			...
			case 2:
				...
				if (intpart < 0 && frac > 0) {
					 ...
					 * order, for binary sort compatibility. Disk value intpart
					 * frac Time value Memory value 800000.00 0 0 00:00:00.00
					 ...
					 * FFFFFFFFFE.FE7960 Formula to convert fractional part from
					 * disk format (now stored in "frac" variable) to absolute
					 * value: "0x100 - frac". To reconstruct in-memory value, we
					 * shift to the next integer value and then substruct
					 * fractional part.
					 */
			...
			case 4:
				...
				if (intpart < 0 && frac > 0) {
					/*
					 * Fix reverse fractional part order: "0x10000 - frac". See
					 * comments for FSP=1 and FSP=2 above.
					 */
					intpart++; /* Shift to the next integer value */
		...
		case BinlogEvent.MYSQL_TYPE_ENUM: {
			...
			case 2:
				...
				break;
			default:
				throw new IllegalArgumentException("!! Unknown ENUM packlen = "
						+ len);
			}
		...
		case BinlogEvent.MYSQL_TYPE_BLOB: {
			...
			default:
				throw new IllegalArgumentException("!! Unknown BLOB packlen = "
						+ meta);
			}
			break;
		}
		...
		case BinlogEvent.MYSQL_TYPE_GEOMETRY: {
			...
			logger.warn(String
					.format("!! Unsupport column type MYSQL_TYPE_GEOMETRY: meta=%d (%04X), len = %d",
							meta, meta, len));
			javaType = Types.BINARY;
			value = binary;
			length = len;
		...
		default:
			logger.error(String
					.format("!! Don't know how to handle column type=%d meta=%d (%04X)",
							type, meta, meta));
			javaType = Types.OTHER;
			value = null;
			length = 0;
