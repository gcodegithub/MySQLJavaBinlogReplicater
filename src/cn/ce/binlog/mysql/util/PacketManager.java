...
public abstract class PacketManager {
	...
	public static HeaderPacket readHeader(SocketChannel ch, int len)
			throws Exception {
	...
	public static void write(SocketChannel ch, byte[] body, byte packetSeqNumber)
			throws IOException {
		HeaderPacket header = new HeaderPacket();
		header.setPacketBodyLength(body.length);
		header.setPacketSequenceNumber(packetSeqNumber);
		write(ch, new ByteBuffer[] { ByteBuffer.wrap(header.toBytes()),
