...
/**
 * This class implements a task to recover assignments after a primary master
 * crash. The main idea is to determine the tasks that have already been
 * assigned and assign the ones that haven't
 * 
 */
public class RecoveredAssignments {
	private static final Logger LOG = LoggerFactory
			.getLogger(RecoveredAssignments.class);
	...
	private cn.ce.binlog.zk.RecoveryCallback cb;
	 ...
	 */
	public void recover(cn.ce.binlog.zk.RecoveryCallback recoveryCallback) {
	...
	ChildrenCallback workersCallback = new ChildrenCallback() {
		public void processResult(int rc, String path, Object ctx,
			...
			case CONNECTIONLOSS:
				...
				break;
			case OK:
				LOG.info("Getting worker assignments for recovery: "
						+ children.size());

				/*
				 * No worker available yet, so the master is probably let's just
				 * return an empty list.
				 */
				if (children.size() == 0) {
					LOG.warn("Empty list of workers, possibly just starting");
					cb.recoveryComplete(RecoveryCallback.OK,
	...
	ChildrenCallback workerAssignmentsCallback = new ChildrenCallback() {
		public void processResult(int rc, String path, Object ctx,
			...
			case OK:
				...
				/*
				 * If the worker is in the list of active workers, then we add
				 * the tasks to the assignments list. Otherwise, we need to
				 * re-assign those tasks, so we add them to the list of tasks.
				 */
				if (activeWorkers.contains(worker)) {
					...
					for (String task : children) {
						if (!tasks.contains(task)) {
							...
							/*
							 * If the task is still in the list we delete the
							 * assignment.
							 */
				...
				/*
				 * Once we have checked all assignments, it is time to check the
				 * status of tasks
				 */
