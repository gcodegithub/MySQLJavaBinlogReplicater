...
public final class FormatDescriptionLogEvent extends StartLogEventV3 {
	...
	public EventVO genEventVo() {
		...
		vo.setBinlogVersion(this.binlogVersion);
		vo.setServerVersion(serverVersion);
		vo.setNumberOfEventTypes(numberOfEventTypes);
		return vo;
	}

	public FormatDescriptionLogEvent(BinlogEventHeader header,
			LogBuffer buffer, FormatDescriptionLogEvent descriptionEvent)
			throws IOException {
	...
	public FormatDescriptionLogEvent(final int binlogVersion) {
		...
		case 4: /* MySQL 5.0 */
			...
			/*
			 * We here have the possibility to simulate a master of before we
			 * changed the table map id to be stored in 6 bytes: when it was
			 * stored in 4 bytes (=> post_header_len was 6). This is used to
			 * test backward compatibility. This code can be removed after a few
			 * months (today is Dec 21st 2005), when we know that the 4-byte
			 * masters are not deployed anymore (check with Tomas Ulin first!),
			 * and the accompanying test (rpl_row_4_bytes) too.
			 */
		...
		case 3: /* 4.0.x x>=2 */
			...
			/*
			 * The first new event in binlog version 4 is Format_desc. So any
			 * event type after that does not exist in older versions. We use
			 * the events known by version 3, even if version 1 had only a
			 * subset of them (this is not a problem: it uses a few bytes for
			 * nothing but unifies code; it does not make the slave detect less
			 * corruptions).
			 */
	...
	public static void doServerVersionSplit(String serverVersion,
			int[] versionSplit) {
