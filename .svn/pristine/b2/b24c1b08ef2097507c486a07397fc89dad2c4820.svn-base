...
public class LogBuffer {
	...
	public final int getBeUint24() {
		...
		return ((0xff & buf[position++]) << 16)
				| ((0xff & buf[position++]) << 8) | (0xff & buf[position++]);
	}
	...
	public final int getBeInt32(final int pos) {
		...
		return (0xff & buf[position + 3]) | ((0xff & buf[position + 2]) << 8)
				| ((0xff & buf[position + 1]) << 16) | ((buf[position]) << 24);
	}
	...
	public final int getBeInt32() {
		...
		byte[] buf = buffer;
		return ((buf[position++]) << 24) | ((0xff & buf[position++]) << 16)
				| ((0xff & buf[position++]) << 8) | (0xff & buf[position++]);
	}
	...
	public final BigInteger getUlong64(final int pos) {
		...
		return (long64 >= 0) ? BigInteger.valueOf(long64) : BIGINT_MAX_VALUE
				.add(BigInteger.valueOf(1 + long64));
	}
